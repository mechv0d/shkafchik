...
const ItemDetailScreen: React.FC = () => {
  ...
  const { deleteItem, toggleFavorite } = useAppActions();
  ...
  const handleDelete = () => {
    Alert.alert('Удалить вещь', 'Вы уверены, что хотите удалить эту вещь из гардероба?', [
    ...
    <ScrollView className="flex-1 bg-background">
      <View className="p-4 space-y-6">
        ...
        <View className="flex-row justify-between items-center">
          <Text className="text-2xl font-bold text-gray-800 flex-1 mr-4">{item.name}</Text>
        ...
        <View className="space-y-4">
          ...
          <View>
            ...
            <Text className="text-primary text-xl font-medium">{formatPrice(item.price)}</Text>
          </View>
        ...
        <View className="space-y-3 pt-4">
          <Button
            ...
            variant="outline"
          />
          <Button title="Удалить вещь" onPress={handleDelete} variant="secondary" />
        </View>
      </View>
    </ScrollView>
  );
