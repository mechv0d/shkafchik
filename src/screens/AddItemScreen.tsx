...
const AddItemScreen: React.FC = () => {
  ...
  const handleAddTag = () => {
    if (
      newTagName.trim() &&
      !tags.find(t => t.name.toLowerCase() === newTagName.trim().toLowerCase())
    ) {
      addTag({
    ...
    <View className="flex-row space-x-2">
      {[1, 2, 3, 4, 5].map(star => (
        ...
        >
          <Text
            className={
              star <= formData.rating ? 'text-yellow-500 text-2xl' : 'text-gray-300 text-2xl'
            }
          >
            Star
          </Text>
    ...
    >
      <View className="space-y-8">
        ...
        <View className="space-y-3">
          ...
          <View className="flex-row items-center justify-center py-2">{renderStars()}</View>
        </View>
            ...
            <View className="flex-row flex-wrap">
              {tags.map(tag => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Tag tag={tag} size="md" selected={selectedTags.includes(tag.id)} />
                </TouchableOpacity>
              ))}
            </View>
          )}
        ...
        >
          ...
          <View className="flex-1">
            <Text className="text-lg font-bold text-text-primary">Добавить в избранное</Text>
            <Text className="text-text-secondary text-sm">Вещь появится в списке избранных</Text>
          </View>
        </TouchableOpacity>
