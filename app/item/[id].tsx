...
export default function ItemDetail() {
  ...
  const [capsulesItemsCount, setCapsulesItemsCount] = useState<
    ...
    try {
      ...
      if (data) {
        ...
        const ratingValue = parseInt(
          data.attributes.find((attr) => attr.attribute_type === "rating")?.value || "0",
          10,
        );
        setRating(Number.isNaN(ratingValue) ? 0 : ratingValue);
      }
      ...
      if (data) {
        ...
        for (const capsule of allCapsules) {
          const capsuleItemIds = await CapsuleItemDAO.getCapsuleItems(
            capsule.id,
          );
  ...
  const handleExport = () => {
    ...
    const exportData = {
      ...
      description:
        item.attributes.find((attr) => attr.attribute_type === "description")
          ?.value || "",
      purchaseDate:
        item.attributes.find((attr) => attr.attribute_type === "purchase_date")
          ?.value || "",
      store:
        item.attributes.find((attr) => attr.attribute_type === "store")
          ?.value || "",
      rating:
        item.attributes.find((attr) => attr.attribute_type === "rating")
          ?.value || "0",
      tags:
        item.tags?.map((tag) => ({ name: tag.name, color: tag.color })) || [],
    ...
    Alert.alert("Экспорт данных", textContent, [
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Избранное",
        newFavoriteStatus
          ? "Вещь добавлена в избранные"
          : "Вещь удалена из избранных",
      );
    } catch (error) {
  ...
  const handleRatingChange = async (newRating: number) => {
    ...
    try {
      ...
      const ratingAttr = item.attributes.find(
        (attr) => attr.attribute_type === "rating",
      );
      if (ratingAttr) {
  ...
  const handleAddDescription = async () => {
    ...
    try {
      ...
      const descAttr = item.attributes.find(
        (attr) => attr.attribute_type === "description",
      );
      if (descAttr) {
  ...
  const handleDelete = () => {
    Alert.alert("Удалить вещь", "Вы уверены, что хотите удалить эту вещь?", [
      { text: "Отмена", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert("Успех", "Вещь удалена");
            router.back();
          } catch (error) {
    ...
    return (
      <View style={itemDetailStyles.section}>
        ...
        {hasTags ? (
          ...
          <Text style={itemDetailStyles.emptyTagsText}>
  ...
  const renderCapsules = () => {
    return (
      <View style={itemDetailStyles.section}>
        ...
        <CapsulesGrid
          capsules={capsules}
          onCapsulePress={(capsule: Capsule) =>
  ...
  const price =
    item.attributes.find((attr) => attr.attribute_type === "price")?.value ||
    "Цена не указана";
  const purchaseDate = item.attributes.find(
    (attr) => attr.attribute_type === "purchase_date",
  )?.value;
  const store = item.attributes.find(
    (attr) => attr.attribute_type === "store",
  )?.value;
  ...
  return (
    <View style={itemDetailStyles.container}>
      ...
      <View
        style={[
          itemDetailStyles.header,
          { zIndex: 1000, position: "relative" },
        ]}
      >
        <TouchableOpacity
          onPress={() => router.back()}
          style={itemDetailStyles.headerButton}
        >
      ...
      >
        ...
        <View style={itemDetailStyles.actionButtons}>
          <TouchableOpacity
            style={itemDetailStyles.editButtonAction}
            onPress={handleEdit}
          >
          ...
          <TouchableOpacity
            style={itemDetailStyles.deleteButtonAction}
        ...
        <View style={itemDetailStyles.content}>
          ...
          <TouchableOpacity style={itemDetailStyles.categories}>
            <Text style={itemDetailStyles.categoriesText}>
              {categoryPath} →{" "}
              <Text style={itemDetailStyles.lastCategoriesText}>
                {item.name}
              </Text>
            </Text>
          </TouchableOpacity>
            ...
            <View style={itemDetailStyles.description}>
              {item.attributes.find(
                ...
                <>
                  ...
                  <TouchableOpacity
                    style={itemDetailStyles.addLink}
      ...
      >
        <View style={itemDetailStyles.modalContainer}>
          <View style={itemDetailStyles.modalContent}>
            ...
            <TextInput
              style={[
                itemDetailStyles.descriptionInput,
            ...
            <View style={itemDetailStyles.modalButtons}>
              <TouchableOpacity
                style={[
                  itemDetailStyles.modalButton,
                  itemDetailStyles.cancelButton,
                ]}
                onPress={() => {
                  setShowDescriptionModal(false);
                  setDescription("");
